...
-- ============================================
-- Travel Urban ALS Time Analysis (±3σ trimming with negative value filter + 90th percentile)
-- ============================================

SET @StartDateTime   = '2024-01-01 00:00:00';
SET @EndDateTime     = '2025-01-01 00:00:00';
SET @GoalTimeSeconds = 360;  -- example: 2 minutes
SET @Sigma           = 3;
SET @Percentile      = 0.9;
...
BaseData AS (
    ...
    WHERE CreateDatetime BETWEEN @StartDateTime AND @EndDateTime
      AND `TravelTime(Sec)` >= 1    -- Removes any "impossible" data
      AND CallGEOFDID = '31D04'     -- Any unit within out distric
      AND Quadrant NOT IN ('AD1218C', 'AD1118A', 'AD1118B', 'AD1118C', 'AD1118D', 'AD1119A', -- Checks for urban quandrant
					   'AD1119B', 'AD1018B', 'AD1018C', 'AD1018D',
                       'AD1019A', 'AD1019B', 'AD0918A', 'AD0918B',
                       'AD0919A', 'AD0919B', 'AD0919C', 'AD0919D')
      AND UnitNumber REGEXP '^(M)[0-9][0-9]' -- checks for is the units responding are medic
      AND IncidentFilter = 'IncidentRow'
      AND FinalCallType IN ('AWWX', 'MCI', 'MEDX', 'MVCE', 'MVCF', 'MVCM', 
							 'MVCP', 'MVCT', 'COAM', 'MAA', 'MED',
                             'MED1', 'MED2')
),
...
GoalStats AS (
    SELECT
        100.0 * SUM(travel_time_sec <= @GoalTimeSeconds) / COUNT(*) AS pct_meeting_goal -- Calculates the percentage of calls that meet the goal
    FROM TrimmedData
)
