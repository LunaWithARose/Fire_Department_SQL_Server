...
SET @GoalTimeSeconds = 120;  -- example: 8 minutes
SET @Sigma = 3;
...
BaseData AS (
    ...
    WHERE CreateDatetime BETWEEN @StartDateTime AND @EndDateTime
      AND `TurnoutTime(Sec)` IS NOT NULL
      AND FDID = '31D04'
),
...
TrimmedData AS (
    ...
    WHERE b.turnout_time_sec BETWEEN
...
TrimmedStats AS (
    SELECT
        ...
        STDDEV(turnout_time_sec) AS stddev_dispatch_time,
...
GoalStats AS (
    SELECT
        100.0 * SUM(turnout_time_sec <= @GoalTimeSeconds)
