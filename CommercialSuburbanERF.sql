...
BaseUnits AS (
    ...
    WHERE f.CreateDatetime BETWEEN @StartDateTime AND @EndDateTime
      ...
      AND f.FinalCallType IN ('FC', 'FCC')
	  AND f.Quadrant NOT IN ('AD1218C', 'AD1118A', 'AD1118B', 'AD1118C', 'AD1118D', 'AD1119A', -- Checks for urban quandrant
...
SELECT 
    ...
    SEC_TO_TIME(TIMESTAMPDIFF(
        ...
        f.DispatchDateTime, 
        t.ArriveDateTime
    )) AS total_time_22nd_firefighter,
    f.DispatchDateTime,
    t.ArriveDateTime,
    f.UnitNumber as First_unit_on_scene,
    t.UnitNumber as 22nd_unit_on_scene,
    f.FinalCallType
FROM FirstFireFighter f
JOIN TwentysecondFirefighter t
